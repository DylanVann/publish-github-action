...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const hasYarnLock = yield fs.exists('yarn.lock');
            const packageManager = hasYarnLock ? 'yarn' : 'npm';
            const hasBuildScript = json.scripts && json.scripts.build;
            if (hasBuildScript) {
                yield exec.exec(`${packageManager} install`);
                yield exec.exec(`${packageManager} run build`);
                yield exec.exec('rm -rf node_modules');
                yield exec.exec(`${packageManager} install --production`);
            }
            else {
                yield exec.exec(`${packageManager} install --production`);
            }
            ...
            if (json.files) {
                yield exec.exec(`git add -f ${json.files.join(' ')}`);
            }
